...
CATEGORY_OPTIONS = {
    ...
    "job_role": [
        "Backend Developer", "Data Scientist", "DevOps", "Frontend Developer",
        "ML Engineer", "Product Manager", "QA Engineer", "Software Engineer",
    ],
...
def validate_and_build_dataframe(payload: dict[str, Any]) -> pd.DataFrame:
    ...
    # -----------------------------------------------------------------------
    # IMPORTANT FIX: We return the raw 23 columns with their raw values.
    # PyCaret's predict_model is responsible for one-hot encoding and scaling.
    # -----------------------------------------------------------------------
...
def validate_batch_dataframe(uploaded_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if duplicates:
        raise BatchValidationError(["Some column names become duplicates after normalisation: " + ", ".join(duplicates)])
    ...
    if missing:
        raise BatchValidationError(["Missing required columns: " + ", ".join(missing) + "."])
    ...
    for feature in FEATURES:
        ...
        if feature in CATEGORY_OPTIONS:
            ...
            if invalid.any():
                ...
                errors.append(f"{label}: invalid or blank value at CSV row(s) {rows}.")
            cleaned[feature] = canonical
            continue

        if feature in BINARY_FEATURES:
            text = raw.astype("string").str.strip().str.casefold()
            binary_map = {"0": 0, "0.0": 0, "no": 0, "false": 0, "n": 0, "1": 1, "1.0": 1, "yes": 1, "true": 1, "y": 1}
        ...
        if out_of_range.any():
            ...
            errors.append(f"{label}: value outside {spec['min']}–{spec['max']} at CSV row(s) {rows}.")
        if non_integer.any():
...
def predict_batch(
...
) -> tuple[list[Any], list[str], list[Optional[float]]]:
    ...
    for start in range(0, len(data_unseen), BATCH_CHUNK_SIZE):
        ...
        try:
            ...
            confidences.extend(
                np.max(probability_matrix, axis=1)
                .astype(float)
                .tolist()
            )
        except Exception:
...
# ---------------------------------------------------------------------------
def render_page(form_values, prediction=None, error=None, batch_error=None, batch_result=None):
...
def predict():
    ...
    try:
        ...
        return render_page(submitted_values, error=f"Internal error: {str(exc)}"), 500
...
def batch_template():
    ...
    second_row.update({
        "age": 29, "gender": "Female", "job_role": "Data Scientist",
        "experience_years": 3.5, "company_size": "Startup", "work_mode": "Remote",
        "work_hours_per_week": 52.0, "overtime_hours": 8.0, "stress_level": 7.2,
        "sleep_hours": 5.8,
    })
...
def batch_predict():
    ...
    if uploaded_file is None or not uploaded_file.filename:
        return render_page(DEFAULT_VALUES.copy(),
                           batch_error=["Choose a CSV file before starting the batch prediction."]), 400
    if not uploaded_file.filename.lower().endswith(".csv"):
        return render_page(DEFAULT_VALUES.copy(),
                           batch_error=["Only .csv files are supported for batch prediction."]), 400
    ...
    try:
        ...
        preview_columns = [c for c in ["age", "job_role", "work_mode", "work_hours_per_week",
                                       "stress_level", "predicted_burnout_level", "prediction_confidence"]
        ...
        batch_result = {
            "filename": uploaded_file.filename, "rows": len(result_df),
            "low": int(counts["Low"]), "moderate": int(counts["Moderate"]), "high": int(counts["High"]),
            "average_confidence": (float(np.mean(valid_confidences)) if valid_confidences else None),
            "preview_columns": preview_columns, "preview_rows": preview_df.to_dict(orient="records"),
            "preview_limit": BATCH_PREVIEW_ROWS, "download_token": token,
        }
        return render_page(DEFAULT_VALUES.copy(), batch_result=batch_result)

    except UnicodeDecodeError:
        return render_page(DEFAULT_VALUES.copy(),
                           batch_error=["The CSV could not be read as UTF-8 text."]), 400
    except pd.errors.EmptyDataError:
        return render_page(DEFAULT_VALUES.copy(), batch_error=["The uploaded CSV is empty."]), 400
    except pd.errors.ParserError as exc:
        return render_page(DEFAULT_VALUES.copy(),
                           batch_error=[f"The CSV structure could not be parsed: {exc}"]), 400
    except BatchValidationError as exc:
        return render_page(DEFAULT_VALUES.copy(), batch_error=exc.messages), 400
    except Exception as exc:
        app.logger.exception("Batch prediction failed")
        return render_page(DEFAULT_VALUES.copy(),
                           batch_error=[f"Batch prediction failed: {str(exc)}"]), 500
...
def upload_too_large(_error):
    return render_page(DEFAULT_VALUES.copy(),
                       batch_error=[f"The CSV is too large. The maximum upload size is {MAX_UPLOAD_MB} MB."]), 413
